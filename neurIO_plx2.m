function [ file ] = neurIO_plx( varargin )
%NEURIO_PLX reads binary data from .PLX files
%   Inputs
%       plx_path: Path to plexon file, must be a readable by matlab
%       channels: 
%
%
%

plx_path = fullfile(pwd, 'test', 'plexon_sample.plx');
...
v = 0; %Valid channel count

for i = 1:info.number_analog_channels

    try
        [sampling_rate, number_samples, time_stamps, fragment_datapoints, sampled_analog] = getAnalog(fID, i);

        if number_samples >= min_samples

        v=v+1; %Bump up valid channel count
        file(v).path = plx_path;
        file(v).type = 'analog';
        file(v).fs = sampling_rate;
        file(v).ns = number_samples;
        file(v).ts = time_stamps;
        file(v).ts = fragment_datapoints;
        file(v).analog = sampled_analog;
        file(v).fileHeader = info;
        file(v).chanHeader.block_order = i;

        clear sampling_rate number_samples time_stamps fragment_datapoints sampled_analog
        else
...
for i = 1:info.number_event_channels
    try
        [number_samples, time_stamps, event_values] = getEvents(fID, i);
        
        if number_samples >= min_events
            v=v+1; %Bump up valid channel count
            ...
            file(v).ns = number_samples;
            file(v).ts = time_stamps;
            file(v).ev = event_values;
            file(v).fileHeader = info;
            file(v).chanHeader.block_order = i;
            
            clear  number_samples time_stamps event_values
        else
        end
    catch err
...
for i = 1:info.number_dsp_channels
     %try
     v=v+1; %Bump up valid channel count
     ...
     file(v).chanHeader.block_order = i;
     for u = 1:4
        [number_samples, time_stamps] = getTimestamps(fID, i, u);

        if number_samples >= min_samples

        [number_samples, waveform_points, time_stamps, waveforms] = getWaveforms(fID, i, u);
        
        file(v).unit(u).ns = number_samples;
        file(v).unit(u).ts = time_stamps;
        file(v).unit(u).wf = waveforms;
        file(v).unit(u).ws = waveform_points;
        
        

        clear  number_samples time_stamps waveform_points waveformsc
        else
