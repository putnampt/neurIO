...
while feof(fID) == 0
    ...
    elseif type == 5
        ...
        error('Unexpected data block type!');
    end
    ...
    switch type
        case 5 % continuous data block
            ...
            slow(chan_idx).found = 1;
            slow(chan_idx).channel = channel;

            %If this is the f
            if ~isfield(slow(chan_idx), 'number_samples')
                
                slow(chan_idx).number_samples = 0;
                slow(chan_idx).frags = 0;
                slow(chan_idx).number_samples = 0;
                slow(chan_idx).time_stamps = 0;
                slow(chan_idx).fragment_datapoints = 0;
                slow(chan_idx).sampled_analog = 0;
            elseif isempty(slow(chan_idx).number_samples)
                slow(chan_idx).number_samples = 0;
                slow(chan_idx).frags = 0;
                slow(chan_idx).number_samples = 0;
                slow(chan_idx).time_stamps = 0;
            ...
            slow(chan_idx).frags = slow(chan_idx).frags+1;
            slow(chan_idx).number_samples = slow(chan_idx).number_samples + num_words;
            slow(chan_idx).time_stamps(slow(chan_idx).frags) = ts/fs;
            slow(chan_idx).fragment_datapoints(slow(chan_idx).frags) = num_words;
            if count > 0
                if adpos+num_words-1 <= count
                    slow(chan_idx).sampled_analog(adpos:adpos+num_words-1) = wf(1:num_words);
                    adpos = adpos + num_words;
                else
                    for i=1:num_words
                        slow(chan_idx).sampled_analog(adpos) = wf(i); adpos = adpos + 1;
                    end
                end
            else
                slow(chan_idx).sampled_analog = [sampled_analog wf(1, 1:num_words)];
            end
        case 4
        case 1
        otherwise
