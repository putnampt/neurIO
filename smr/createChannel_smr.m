function[freechan]=createChannel_smr(fid,SrcChan,data,dataheader)
...
if strcmp(permission,'rb+')==0
    error('createChannel_smr:  File not opened for writing');
end;

FileH=SONFileHeader(fid);
if (FileH.systemID<6)  & (strcmp(datatype,'int16')~=1)      % ... make sure it's compatible with the file
    warning('createChannel_smr: SON file is below version 6. RealWave data not allowed. Use SONUpgradeToVersion6.m first');
    return;
end;
...
if (Failed==1)
    error('createChannel_smr: No free space exists in the file to write a channel header');
end;
