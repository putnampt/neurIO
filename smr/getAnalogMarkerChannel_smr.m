function[data,h]=getAnalogMarkerChannel_smr(fid, chan, varargin)
% getAnalogMarkerChannel_smr reads an ADCMark channel from a file.
%
% [DATA {, HEADER}]=getAnalogMarkerChannel_smr(FID, CHAN{, START{, STOP{, OPTIONS}}})
% FID is the matlab file handle, CHAN is the channel number (1=max)
% 
% [DATA, HEADER]=getAnalogMarkerChannel_smr(FID, 1{, OPTIONS})
%       reads all the data on channel 1
% [DATA, HEADER]=getAnalogMarkerChannel_smr(FID, 1, 10{, OPTIONS})
%       reads disc block 10 for continuous data or epoch 10 for triggered
%       data
% [DATA, HEADER]=getAnalogMarkerChannel_smr(FID, 1, 10, 20{, OPTIONS})
...
% 'scale' - calls to apply the channel scale and offset to DATA
...
% [data, header]=getAnalogMarkerChannel_smr(fid, 1, 20, 40, options{:})
...
%   scaling and applying the offset stored on disc.
%
...
if Info.kind ~=6
    warning('getAnalogMarkerChannel_smr: Channel %d No data or wrong channel type', chan);
    data=[];
    h=[];
    return;
end;
