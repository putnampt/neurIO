function[data,h]=getAnalog_smr(fID, chan, varargin)
% getAnalog_smr reads an ADC (waveform) channel from a  file.
%
% [DATA {, HEADER}]=getAnalog_smr(FID, CHAN{, START{, STOP{, OPTIONS}}})
% FID is the matlab file handle, CHAN is the channel number (1=max)
% 
% [DATA, HEADER]=getAnalog_smr(FID, 1{, OPTIONS})
%       reads all the data on channel 1
% [DATA, HEADER]=getAnalog_smr(FID, 1, 10{, OPTIONS})
%       reads disc block 10 for continuous data or epoch 10 for triggered
%       data
% [DATA, HEADER]=getAnalog_smr(FID, 1, 10, 20{, OPTIONS})
...
% 'scale' - calls convertToDouble_smr to apply the channel scale and offset to DATA
...
% 
% Examples:
% [data, header]=getAnalog_smr(fID, 1, 'ticks')
...
% [data, header]=getAnalog_smr(fID, 1, 200, 399, options{:})
...
%    the offset stored on disc via convertToDouble_smr. If sampling was
...
%         system: Matoff version identifier
...
if Info.kind ~=1
    warning('getAnalog_smr: Channel %d No data or wrong channel type', chan);
    data=[];
    h=[];
    return;
end;
...
% Make sure we are in range if using START and STOP    
        if (startEpoch>Info.blocks || startEpoch>endEpoch)
            ...
            h=[];
            close(progbar);
            warning('getAnalog_smr: Invalid START and/or STOP')
            return;
        end;
...
if ScaleData==1
    ...
    [data,h]=convert2Double_smr(data,h);
end;
