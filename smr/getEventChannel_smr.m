function[data,h]=getEventChannel_smr(fID, chan, varargin)
% getEventChannel_smr reads an event channel from a SON file
%
% DATA{, H}]=getEventChannel_smr(fID, CHAN{, START{, STOP{, OPTIONS}}})
%
...
%
% e.g.
% [data,h]=getEventChannel_smr(fID, 1, 10, 11, 'microseconds')
%     reads Block 10-11 of Channel 1 and returns that data in microseconds
% [data,h]=getEventChannel_smr(fID, 1, 'ticks')
...
if(Info.kind < 2 || Info.kind > 4 ) 
    warning('getEventChannel_smr: Channel #%d No data or not an event channel',chan);
    data=[];
    h=[];
    return;
end;
...
[data,h.TimeUnits]=convertSamplesToTime_smr(fID, data, varargin{:});      % Convert time
