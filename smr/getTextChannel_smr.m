function[data,h]=getTextChannel_smr(fID, chan, varargin)
% getTextChannel_smr reads a marker channel from a file.
...
if(Info.kind~=8) 
    warning('getTextChannel_smr: Channel %d No data or not a TextMark channel',chan);
    data=[];
    h=[];
    return;
    return;
...
if(nargout>1)
    ...
    h.system=['openPhys' num2str(FileH.systemID)];
    h.FileChannel=chan;
